...
export class CodexAdapter {
  ...
  capabilities() {
    return { harness: this.name, headless: true, structuredEvents: true, resumeSession: true, nativeSessionId: true, hooks: false };
  }
...
export async function runChild({ binary, args, cwd, env, spawnImpl, parseJsonLines, onNativeEvent }) {
  let child;
  try {
    child = spawnImpl(binary, args, { cwd, env, stdio: ["ignore", "pipe", "pipe"], windowsHide: true });
  } catch (error) {
    return { exitCode: 127, signal: null, sessionId: null, output: "", stderr: String(error), reason: "binary_missing" };
  }
  let stdout = "", stderr = "", buffer = "", sessionId = null;
  const errors = [], agentMessages = [];
  const consumeLine = async (line) => {
    if (!line.trim()) return;
    if (!parseJsonLines) { await onNativeEvent({ type: "stdout", text: truncate(line, 50_000) }); return; }
    try {
      ...
      if (event.type === "item.completed" && event?.item?.type === "agent_message") agentMessages.push(String(event.item.text ?? ""));
  ...
  child.stdout?.on?.("data", (chunk) => {
    stdout += String(chunk); if (stdout.length > 500_000) stdout = stdout.slice(-500_000);
    buffer += String(chunk);
    let idx;
    while ((idx = buffer.indexOf("\n")) >= 0) { const line = buffer.slice(0, idx); buffer = buffer.slice(idx + 1); void consumeLine(line); }
  });
  child.stderr?.on?.("data", (chunk) => { stderr += String(chunk); if (stderr.length > 300_000) stderr = stderr.slice(-300_000); });
